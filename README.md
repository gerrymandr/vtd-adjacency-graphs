...
### Folder structure
In the root directory, you'll find a folder for each[*](#whats-missing) state, named by the 2-digit FIPS code for that state. You can find a matching of state names to FIPS codes [here](https://www.mcc.co.mercer.pa.us/dps/state_fips_code_listing.htm).
...
- __Information from the Census shapefiles__. Each node has the following attributes:
...
The [TIGER2012/VTD/](https://www2.census.gov/geo/tiger/TIGER2012/VTD/) folder on the Census website did not have a shapefile for Rhode Island, so __we currently do not have a VTD graph for Rhode Island__. We have another source for Rhode Island shapefiles that we will add to the repository soon.
